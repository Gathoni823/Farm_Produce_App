...
@Composable
fun UpdateFarmerScreen(navController: NavController,id:String){
    val imageUri = rememberSaveable() {
        mutableStateOf<Uri?>( null)
    }
    val painter = rememberAsyncImagePainter(
        ImageRequest.Builder(LocalContext.current)
            .data(data = imageUri.value ?: R.drawable.placeholder).apply(block = fun ImageRequest.Builder.() {
                crossfade(true)
            }).build()
    )
