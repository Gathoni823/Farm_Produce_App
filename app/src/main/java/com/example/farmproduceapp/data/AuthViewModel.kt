...
import com.example.farmproduceapp.navigation.ROUTE_HOME_ONE
import com.example.farmproduceapp.navigation.ROUTE_HOME_TWO
...
class AuthViewModel: ViewModel() {
    ...
    fun signup(
        ...
        mAuth.createUserWithEmailAndPassword(email, password).addOnCompleteListener { task ->
            ...
            if (task.isSuccessful) {
                ...
                val userData = Users.SignupModel(
                    firstName = firstName,
                    lastName = lastName,
    ...
    fun saveUserToDatabase(
        userId: String, userData: Users.SignupModel, navController: NavController, context: Context
    ) {
        val regRef = FirebaseDatabase.getInstance().getReference("Clients/$userId")
    ...
    fun viewUsers(
        clients: SnapshotStateList<Users.SignupModel>,
        context: Context
    ): SnapshotStateList<Users.SignupModel> {
        val ref = FirebaseDatabase.getInstance().getReference("Clients")
        ref.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                clients.clear()
                for (snap in snapshot.children) {
                    val value = snap.getValue(Users.SignupModel::class.java)
                    if (value != null) {
                        clients.add(value)
                    }
                }
...
//        val databaseReference = FirebaseDatabase.getInstance().getReference("Clients/$userId")
//
//        // Create a SignupModel object with the new fields
//        val updatedClient = Users.SignupModel(
//            firstName = firstName,
//            lastName = lastName,
//            phoneNumber = phoneNumber,
//            email = email,
    ...
    fun login(email: String, password: String, navController: NavController, context: Context) {
        ...
        mAuth.signInWithEmailAndPassword(email, password).addOnCompleteListener { task ->
            ...
            if (task.isSuccessful) {
                ...
                val userRef = FirebaseDatabase.getInstance().getReference("Clients/$userId")
        ...
        userRef.get().addOnCompleteListener { task ->
            if (task.isSuccessful) {
                ...
                if (userSnapshot.exists()) {
                    ...
                    showToast("Client Details: $firstName $lastName", context)

                    // Here, you could update your UI with these values (e.g., through a LiveData or StateFlow)
                } else {
                    showToast("Client not found in database", context)
                }
            } else {
                showToast("Failed to retrieve client details: ${task.exception?.message}", context)
            }
        }
    }
